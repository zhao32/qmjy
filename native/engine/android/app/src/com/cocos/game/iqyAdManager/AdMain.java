...
public class AdMain {
    ...
    public void SDK_Init(String appId) {
        ...
        QySdk.init(m_ctx, QySdkConfig.newAdConfig()
                        .appId(appId)
                        .appName(m_ctx.getString(R.string.app_name))
                        .debug(true)
                        .build(),
                new QySdk.InitCallBack() {
                    @Override
                    public void success() {
                        // 初始化成功
                        if (m_isInit) return;
                        DebugPrintE("Android: iqy 加载SDK成功");
                        m_isInit = true;
                        SDK_StartLoad();
                        // 在初始化成功回调之后进行广告加载
                    ...
                    public void fail(int i, String s) {
                        // 初始化失败
                        DebugPrintE("Android: iqy 加载SDK错误 code: %s, msg: %s", i, s);
                        m_isInit = false;
                        if (m_adMainCallBack.sdkInitCallBack != null) {
    ...
    public void handlerAd(Context context, String event, JSONObject data) {
        if (TextUtils.equals(event, "initAd")) {
            ...
            String unitId = data.getString("unitId");
            long channel = data.getLongValue("channel");
            int top = 0;
            if (data.containsKey("style")) {
                JSONObject style = data.getJSONObject("style");
                if (style.containsKey("top")) top = style.getIntValue("top");
            }
            loadAdHandler(AdBanner.getInstance().LoadAd(unitId, channel, top), event, (s) -> AdBanner.getInstance().ShowAd(s));
            ...
            String unitId = data.getString("unitId");
            long channel = data.getLongValue("channel");
            loadAdHandler(AdReward.getInstance().LoadAd(unitId, channel), event, (s) -> AdReward.getInstance().ShowAd(s));
        } else if (TextUtils.equals(event, "feedAd")) {
            DebugPrintE("Android: iqy 不支持 %s", "feed");
        } else if (TextUtils.equals(event, "drawAd")) {
            DebugPrintE("Android: iqy 不支持 %s", "draw");
        }
    }
