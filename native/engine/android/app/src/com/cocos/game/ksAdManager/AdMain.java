        ...
        SdkConfig config = new SdkConfig.Builder()
                ...
                .setInitCallback(new KsInitCallback() {
                    ...
                    public void onSuccess() {
                        DebugPrintE("Android: ks SDK 加载成功");
                        KsAdSDK.start();
                    }

                    @Override
                    public void onFail(int code, String msg) {
                        DebugPrintE("Android: ks SDK 加载错误 code:%s, msg: %s", code, msg);
                        m_isInit = false;
                ...
                .setStartCallback(new KsInitCallback() {
                    ...
                    public void onSuccess() {
                        if (m_isInit) return;
                        DebugPrintE("Android: ks SDK start成功");
                        m_isInit = true;
                        SDK_StartLoad();
                        // 在初始化成功回调之后进行广告加载
                        if (m_adMainCallBack.sdkInitCallBack != null) {
                    ...
                    @Override
                    public void onFail(int code, String msg) {
                        DebugPrintE("Android: ks SDK start错误 code:%s, msg: %s", code, msg);
                        m_isInit = false;
                        if (m_adMainCallBack.sdkInitCallBack != null) {
                            m_adMainCallBack.sdkInitCallBack.onError(code, msg);
                        }
