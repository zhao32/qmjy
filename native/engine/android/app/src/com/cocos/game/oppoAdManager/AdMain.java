...
public class AdMain {
    ...
    public void SDK_Init(String appId) {
        ...
        InitParams initParams = new InitParams.Builder().setDebug(true).build();
        MobAdManager.getInstance().init(m_ctx, appId, initParams, new IInitListener() {
            @Override
            public void onSuccess() {
                // 初始化成功
                if (m_isInit) return;
                DebugPrintE("Android: oppo 加载SDK成功");
                m_isInit = true;
                SDK_StartLoad();
                // 在初始化成功回调之后进行广告加载
            ...
            public void onFailed(String s) {
                // 初始化失败
                DebugPrintE("Android: oppo 加载SDK错误 msg: %s", s);
                m_isInit = false;
                if (m_adMainCallBack.sdkInitCallBack != null) {
    ...
    public void handlerAd(Context context, String event, JSONObject data) {
        if (TextUtils.equals(event, "initAd")) {
            ...
            loadAdHandler(AdSplash.getInstance().LoadAd(unitId, title, desc), event, (s) -> AdSplash.getInstance().ShowAd(s));
            ...
            DebugPrintE("Android: oppo 不支持 %s", "feed");
        } else if (TextUtils.equals(event, "drawAd")) {
            DebugPrintE("Android: oppo 不支持 %s", "draw");
        }
    }
