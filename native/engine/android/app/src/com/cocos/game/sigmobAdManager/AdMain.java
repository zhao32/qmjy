...
public class AdMain {
    ...
    public void SDK_Init(String appId, String appKey) {
        ...
        WindAdOptions options = getWindAdOptions(appId, appKey);
        ads.init(m_ctx, options, new OnInitializationListener() {
            @Override
            public void onInitializationSuccess() {
                if (m_isInit) return;
                DebugPrintE("Android: sigmob 加载SDK成功");
                SDK_StartLoad();
            }

            @Override
            public void onInitializationFail(String msg) {
                DebugPrintE("Android: sigmob 加载SDK错误 msg: %s", msg);
                m_isInit = false;
                if (m_adMainCallBack.sdkInitCallBack != null) {
                    m_adMainCallBack.sdkInitCallBack.onError(0, msg);
                }
            }
        });
    }
    ...
    @NonNull
    private static WindAdOptions getWindAdOptions(String appId, String appKey) {
    ...
    public void SDK_StartLoad() {
        ads.start(new OnStartListener() {
            ...
            public void onStartSuccess() {
                if (m_isInit) return;
                DebugPrintE("Android: sigmob 启动SDK成功");
                m_isInit = true;
                // 在初始化成功回调之后进行广告加载
                if (m_adMainCallBack.sdkInitCallBack != null) {
            ...
            @Override
            public void onStartFail(String msg) {
                DebugPrintE("Android: sigmob 启动SDK错误 msg: %s", msg);
                m_isInit = false;
                if (m_adMainCallBack.sdkInitCallBack != null) {
                    m_adMainCallBack.sdkInitCallBack.onError(0, msg);
                }
    ...
    public void handlerAd(Context context, String event, JSONObject data) {
        if (TextUtils.equals(event, "initAd")) {
            ...
            DebugPrintE("Android: sigmob 不支持 %s", "banner");
            ...
            DebugPrintE("Android: sigmob 不支持 %s", "feed");
        } else if (TextUtils.equals(event, "drawAd")) {
            DebugPrintE("Android: sigmob 不支持 %s", "draw");
        }
    }
